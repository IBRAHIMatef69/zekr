...
class AfterPrey extends StatelessWidget {
  AfterPrey({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: ListView.builder(
          ...
          itemCount: controller.prayZekrScreen.length == 0
              ? 0
              : controller.prayZekrScreen.length,
          itemBuilder: (_, index) {
            return GetBuilder<OutHomeController>(
              builder: (_) {
                return UnCounted(
                    zekrBody: controller.prayZekrScreen[index].zekr.toString(),
                    category:
                        controller.prayZekrScreen[index].category.toString());
              },
            );
          },
        ));
